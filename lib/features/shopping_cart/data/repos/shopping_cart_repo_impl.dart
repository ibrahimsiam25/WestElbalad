...
class ShoppingCardRepoImpl extends ShoppingCartRepo{
  ...
  @override
  Future<Either<Failure, void>> addOrder({required Map<String, dynamic> data,}) async{
   try {
    String documentId = generateUniqueId();
        List<Map<String, dynamic>> ordersData= await getListOfMapsFromSharedPref(kOrder);
        final List<PhoneEntites> orders = ordersData.map((ordersData) {
      return PhoneModel.fromMap(ordersData);
    }).toList();

    UserInfoForOrderEntities userInfoForOrderEntities = UserInfoForOrderEntities(
      id: documentId,
  ...
  await shoppingCartRemoteDataSource.addOrder(
      UserInfoForOrderModel.fromEntity(userInfoForOrderEntities).toMap(), documentId);
